...
bool SCManager::detectLoopClosureSC(int *closestID)
{
    auto result = detectLoopClosureID();
    int SCLoopKeyPre = result.first;
    if (SCLoopKeyPre == -1) //no loop found
        return false;
    *closestID = SCLoopKeyPre;
    return true;
}
