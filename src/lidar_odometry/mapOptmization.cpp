...
public:
    ...
    mapOptimization()
    {
        ...
        std::string pcdDirectory = std::getenv("HOME") + savePCDDirectory;
        saveSCDDirectory = pcdDirectory + "SCDs/"; // SCD: scan context descriptor 
        saveNodePCDDirectory = pcdDirectory + "Scans/";
        std::cout << "pcdDirectory: " << pcdDirectory << std::endl;
        std::cout << "saveSCDDirectory: " << saveSCDDirectory << std::endl;
        std::cout << "saveNodePCDDirectory: " << saveNodePCDDirectory << std::endl;
        // giseop
        // create directory and remove old files;
        int unused = system((std::string("exec rm -r ") + pcdDirectory).c_str());
        unused = system((std::string("mkdir ") + pcdDirectory).c_str());
        ...
        pgSaveStream = std::fstream(pcdDirectory + "singlesession_posegraph.g2o", std::fstream::out);
        pgTimeSaveStream = std::fstream(pcdDirectory + "times.txt", std::fstream::out); pgTimeSaveStream.precision(dbl::max_digits10);
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    bool saveMapService(lvi_sam::save_mapRequest &req, lvi_sam::save_mapResponse &res)
    {
        ...
        if (req.destination.empty())
            saveMapDirectory = std::getenv("HOME") + savePCDDirectory + "map/";
        else
            saveMapDirectory = std::getenv("HOME") + req.destination + "map/";
    ...
    void performLoopClosure()
    {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        // find keys
        int loopKeyCur = -1, loopKeyPre = -1;
        int externalLoopKeyCur = -1, externalLoopKeyPre = -1;
        int RSLoopKeyCur = -1, RSLoopKeyPre = -1;
        int SCLoopKeyCur = copy_cloudKeyPoses3D->size() - 1, SCLoopKeyPre = -1;
        int BOW3DLoopKeyCur = copy_cloudKeyPoses3D->size() - 1, BOW3DLoopKeyPre = -1;
        std::string loop_flag = "none";
        if (detectLoopClosureExternal(&externalLoopKeyCur, &externalLoopKeyPre) == false)
        {
            if (loopClosureMode == 0) // use RS loop closure detection
            {
                if (!detectLoopClosureDistance(&RSLoopKeyCur, &RSLoopKeyPre)) return;
                loopKeyCur = RSLoopKeyCur;
                loopKeyPre = RSLoopKeyPre;
                loop_flag = "RS";
                std::cout << "RS loop found! between " << loopKeyCur << " and " << loopKeyPre << "." << std::endl;
            }
            else if (loopClosureMode == 1) // use SC loop closure detection
            {
                if (!scManager.detectLoopClosureSC(&SCLoopKeyPre)) return;
                loopKeyCur = SCLoopKeyCur;
                loopKeyPre = SCLoopKeyPre;
                loop_flag = "SC";
                std::cout << "SC loop found! between " << loopKeyCur << " and " << loopKeyPre << "." << std::endl;
            }
            else if (loopClosureMode == 2) // use BoW3D loop closure detection
            {
                if (pBoW3D->mvFrames.size() > 10)
                {
                    Eigen::Matrix3d loopRelR;
                    Eigen::Vector3d loopRelt;
                    pBoW3D->retrieve(BOW3DLoopKeyPre, loopRelR, loopRelt);
                    if (BOW3DLoopKeyPre != -1)
                    {
                        loopKeyCur = pBoW3D->mvFrames.back()->mnId;
                        loopKeyPre = BOW3DLoopKeyPre;
                        loop_flag = "RS"; //validation process is the same as RS
                        std::cout << "BOW3D loop found! between " << loopKeyCur << " and " << loopKeyPre << "." << std::endl;
                    }
                }
                if (detectLoopClosureDistance(&RSLoopKeyCur, &RSLoopKeyPre)) // rs is the priority
                {
                    loopKeyCur = RSLoopKeyCur;
                    loopKeyPre = RSLoopKeyPre;
                    loop_flag = "RS";
                    std::cout << "RS loop found! between " << RSLoopKeyCur << " and " << RSLoopKeyPre << "." << std::endl;
                }
            }
            // if (detectLoopClosureDistance(&RSLoopKeyCur, &RSLoopKeyPre) == false || scManager.detectLoopClosureSC(&SCLoopKeyPre) == false)
            //     return;
            // if (RSLoopKeyCur == -1 || RSLoopKeyPre == -1 || SCLoopKeyCur == -1 || SCLoopKeyPre == -1)
            //     return;
            // if (RSLoopKeyCur != SCLoopKeyCur || RSLoopKeyPre != SCLoopKeyPre)
            //     return;
            // loopKeyCur = SCLoopKeyCur;
            // loopKeyPre = SCLoopKeyPre;
            // loop_flag = "SC";
            // std::cout << "Both RS loop and SC loop found! between " << loopKeyCur << " and " << loopKeyPre << "." << std::endl;

            // if (detectLoopClosureDistance(&RSLoopKeyCur, &RSLoopKeyPre) == false && scManager.detectLoopClosureSC(&SCLoopKeyPre) == false)
            //     return;
            // if (SCLoopKeyCur != -1 && SCLoopKeyPre != -1)
            // {
            //     loopKeyCur = SCLoopKeyCur;
            //     loopKeyPre = SCLoopKeyPre;
            //     loop_flag = "RS";
            //     std::cout << "SC loop found! between " << loopKeyCur << " and " << loopKeyPre << "." << std::endl;
            // }
            // if (RSLoopKeyCur != -1 && RSLoopKeyPre != -1)
            // {
            //     loopKeyCur = RSLoopKeyCur;
            //     loopKeyPre = RSLoopKeyPre;
            //     loop_flag = "RS";
        ...
        {
            std::cout << "External loop found! between " << externalLoopKeyCur << " and " << externalLoopKeyPre << "." << std::endl;
            // if (detectLoopClosureDistance(&RSLoopKeyCur, &RSLoopKeyPre) == false && scManager.detectLoopClosureSC(&SCLoopKeyPre) == false)
            //     return;
            // if (externalLoopKeyCur == RSLoopKeyCur && externalLoopKeyPre == RSLoopKeyPre)
            // {
            //     loopKeyCur = RSLoopKeyCur;
            //     loopKeyPre = RSLoopKeyPre;
            //     loop_flag = "RS";
            //     std::cout << "Both external loop and RS loop found! between " << loopKeyCur << " and " << loopKeyPre << "." << std::endl;
            // }
            // if (externalLoopKeyCur == SCLoopKeyCur && externalLoopKeyPre == SCLoopKeyPre)
            // {
            //     loopKeyCur = SCLoopKeyCur;
            //     loopKeyPre = SCLoopKeyPre;
            //     loop_flag = "SC";
            //     std::cout << "Both external loop and SC loop found! between " << loopKeyCur << " and " << loopKeyPre << "." << std::endl;
            // }
        }
        ...
        if (loop_flag == "RS")
        {
            ...
            if (icp.hasConverged() == false || icp.getFitnessScore() > historyKeyframeFitnessScore)
                return;
            std::cout << "------ RS validation finish!------" << std::endl;
    ...
    void performRSLoopClosure()
    {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        if (detectLoopClosureExternal(&loopKeyCur, &loopKeyPre) == false)
        {
            ...
            std::cout << "External loop found! between " << loopKeyCur << " and " << loopKeyPre << "." << std::endl;
        }
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void saveKeyFramesAndFactor()
    {
        ...
        if (loopClosureMode == 1) // use SC
        {
            ...
            // const SCInputType sc_input_type = SCInputType::SINGLE_SCAN_FEAT; // change this 
        ...
        else if (loopClosureMode == 2) // use BoW3D
        {
