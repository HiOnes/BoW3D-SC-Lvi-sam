...
namespace BoW3D
{
    BoW3D::BoW3D(LinK3D_Extractor *pLinK3D_Extractor, float thr_, int thf_, int num_add_retrieve_features_) : mpLinK3D_Extractor(pLinK3D_Extractor),
                                                                                                              thr(thr_),
                                                                                                              thf(thf_),
    ...
    {
        Frame *pCurrentFrame = mvFrames.back();
                    ...
                    if (p[j] != 0)
                    {
                        ...
                        else
                        {
                            ...
                            for (auto placesIter = (wordPlacesIter->second).begin(); placesIter != (wordPlacesIter->second).end(); placesIter++)
                            {
                                ...
                                if (frameId - (*placesIter).first < 20)
                                {
                                    continue;
                                }
        ...
        // std::cout << "canditate: " << mScoreFrameID.size() << std::endl;
        ...
        for (auto it = mScoreFrameID.rbegin(); it != mScoreFrameID.rend(); it++)
        {
            ...
            if (loopId == frameId)
            {
                std::cout << "----- same frame,continue ..." << std::endl;
                continue;
            }
                ...
                for (size_t j = 0; j < (size_t)descriptors.cols; j++)
                {
                    ...
                    if (p[j] != 0)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // The interval between the loop and the current frame should be at least 300.
                                if (frameId - (*placesIter).first < 200)
                                {
                                    continue;
                                }
